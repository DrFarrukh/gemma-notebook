...
Source-complete chat fills a per-source evidence budget while preserving adaptive hard bounds: at most 12 chunks/12,000 characters for 1–2 sources, 9/9,000 for 3–5, 6/6,000 for 6–10, and 4/4,800 above 10. It first selects the highest-ranked useful chunk from each distinct named section, then fills remaining budget in rank order while retaining duplicate suppression and character caps. New answers cite supplied evidence with the `C` namespace, such as `[C1]` or `[C2, C5]`; plain numeric references such as `[32]` remain paper bibliography references. Saved older answers retain clickable legacy numeric citations.
